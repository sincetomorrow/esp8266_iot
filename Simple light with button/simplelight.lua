...
function buttonClicked()
    if(state==0)then state = 1; gpio.write(led, gpio.HIGH);
    elseif(state==1)then state = 0; gpio.write(led, gpio.LOW);
    end
...
srv:listen(80,function(conn)
    conn:on("receive", function(conn,request)
        ...
        if(_GET.PIN == "ON1")then
              pwm.close(led); gpio.write(led, gpio.HIGH); state=1; onstate=""; offstate="off"; lampColor = "#D6D245";
        elseif(_GET.PIN == "OFF1")then
              gpio.write(led, gpio.LOW); pwm.close(led); state=0; onstate="off"; offstate=""; lampColor = "#333333";       
        elseif(_GET.PIN == "ON")then
              pwm.setup(led,1000,128); state=1; onstate=""; offstate="off"; lampColor = "#717220";
              pwm.start(led)
        elseif(_GET.PIN == "OFF2")then
            pwm.close(led); state=0; onstate="off"; offstate=""; lampColor = "#333333";
        elseif(gpio.read(led)==1) then
            onstate=""; offstate="off"; lampColor = "#D6D245";
        elseif(gpio.read(led)==0) then
        ...
        buf = buf.."<a href='?PIN=ON'><span class='glyphicon glyphicon-lamp fa-4x' aria-hidden='true' style='color:"..lampColor.."'></span></a>";
